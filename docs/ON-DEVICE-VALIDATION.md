# On-device validation checklist (built-in relay + configuration management)

The relay is a built-in feature of the `tedge` platform module (its scripts live
at `/opt/tedge/bin/relay-*`, config at `/opt/tedge/etc/relay`), not a separate
Router App.
...
- ☐ tedge platform module installed and the device is **online** in Cumulocity
  (`MOD_TEDGE_ENABLED=1`, `MOD_TEDGE_C8Y_URL` set, registered). Relay is part of
  this module — no separate install.
- ☐ `/opt/tedge/etc/relay` has `MOD_RELAY_ENABLED=1` (the default) and
  `MOD_RELAY_OUTPUTS` set (`"out0"` on ICR-1642; `"out0 out1"` on ICR-44xx).
- ☐ `/opt/tedge/etc/init restart` run after editing settings.
...
- ☐ If missing: `ls -l /opt/tedge/operations/c8y/` shows the native files
  `c8y_Relay` and `c8y_RelayArray` (shipped by the platform, not symlinks);
...
- ☐ On start, `relay-seed` published the initial array — the device MO has
  ...
  (`grep relay-seed /var/log/tedge/relay-seed.log`).
...
- ☐ Device → **Configuration** tab lists `tedge.toml`, `system.toml` (from the
  bundle) **and** the `relay` type (registered by the tedge module's `install`).
...
- ☐ With `MOD_RELAY_ENABLED=0` (then `/opt/tedge/etc/init restart`),
  `/opt/tedge/etc/init status` lists `relay-monitor: disabled` and no seed is
  published.
...
- ☐ Repeat sections 1–6 on the other platform (`tedge.v4i.tgz` on ICR-1642,
  `tedge.v4.tgz` on ICR-44xx), confirming the single build works on both.

---

If any hardware step differs (e.g. the output isn't `out0`/`out1`), the only
file to change is `drive_output()` in `modules/tedge/merge/bin/relay-control`
